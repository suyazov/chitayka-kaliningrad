...
- Формы заявок: обработка через `admin-post.php`, обязательны nonce, honeypot-поле, sanitization (`sanitize_text_field`, `sanitize_email`) и согласие на обработку ПДн. Production создаёт лид через ALFACRM; `wp_mail` на `admin_email` — резервный канал при ошибке или отсутствии настройки. Заявки в WordPress БД не хранить.
...
- В WordPress-теме форма обрабатывается через `admin-post.php`: production создаёт лид в ALFACRM, `admin_email` служит резервным каналом; статический preview остаётся демонстрационным.
...
- До интеграции с ALFACRM форма на production обрабатывалась через `admin-post.php`, требовала согласие и отправляла заявку на WordPress `admin_email`; honeypot-smoke проверен без отправки письма клиенту.
- 12.09.2026 серверная интеграция формы с ALFACRM v2 API развёрнута на production: лид создаётся через `customer/create`, временный токен кэшируется на 55 минут и обновляется один раз после HTTP 401, а `wp_mail` остаётся fallback при ошибке или отсутствии конфигурации. Runtime-настройки находятся только в production `wp-config.php`; API-ключ запрещено хранить в Git. Кабинет клиента: `chitayka.s20.online`, рабочий филиал: `1`, источник `Сайт chitayka39.ru`: `15`. Для первой колонки «Не разобрано» `lead_status_ids` намеренно не передаётся: это служебный нулевой этап, отсутствующий в справочнике `lead-status`. Сквозной тест production-формы создал тестовый лид `380` в «Не разобрано» с источником сайта; резервная копия изменённых production-файлов хранится вне Git в `/root/backups/chitayka39.ru/2026-09-12-before-alfacrm/`.
